...
async function main() {
  ...
  // ----------------- Deploy Survey -----------------
  const SurveyFactory = await hardhatEthers.getContractFactory("Survey", wallet);
  ...
  console.log("Survey contract deployed at:", deployedAddress);
  ...
  if (!fs.existsSync(frontendSrcDir)) {
    ...
    try {
      ...
      const targetAbiPath = path.join(abiDir, "Survey.json");
      fs.copyFileSync(artifactPath, targetAbiPath);
      console.log("Copied ABI to frontend/web/src/abi/Survey.json");
    } catch (e) {
      console.warn(
        "Failed to copy ABI automatically. Please copy artifacts/.../Survey.json manually to frontend/web/src/abi/Survey.json",
        e
      );
    }
  }
