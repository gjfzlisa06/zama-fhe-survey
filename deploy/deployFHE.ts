...
async function main() {
  ...
  // Deploy the plain SurveyFHE contract
  const SurveyFactory = await hardhatEthers.getContractFactory("SurveyFHE", wallet);
  const survey = await SurveyFactory.deploy();
  await survey.waitForDeployment();

  const deployedAddress = (survey as any).target || (survey as any).address;
  console.log("SurveyFHE contract deployed at:", deployedAddress);
  ...
  if (!fs.existsSync(frontendConfigDir)) {
    ...
    try {
      ...
      fs.copyFileSync(artifactPath, path.join(targetAbiPath, "SurveyFHE.json"));
      console.log("Copied ABI to frontend/web/src/abi/SurveyFHE.json");
    } catch (e) {
      console.warn(
        "Failed to copy ABI automatically. Please copy artifacts/.../SurveyFHE.json manually to frontend/web/src/abi/SurveyFHE.json",
        e
      );
    }
  }
